...
2. 进入三栏编辑器：左侧是受控组件库，中间是 DAG，右侧是属性、输入、输出和运行详情。拖入节点并连线，确认端口类型不兼容时会被拒绝。演示删除节点会同步清理关联连线，清空画布可以撤销；再演示自动布局、适应画布、全屏和左右面板折叠。
...
11. 查看运行记录，确认默认按最新时间排序，可筛选业务运行、系统验证和状态；新运行显示 12 位定义摘要。最后查看 Pipeline DSL 和 Workflow，确认只有固定 `templateRef`，不存在用户镜像、Shell、ServiceAccount 或原始 Argo YAML。
